...
export default function () {
    ...
    let submissionResponse = http.post("http://localhost:5000/api/submission/create", JSON.stringify(submissionPayload), {
    ...
    if (!checkSubmission) {
        ...
        for (let i = 0; i < tryoutData.length; i++) {
            ...
            let answer = http.post("http://localhost:5000/api/answer/create", JSON.stringify(answerPayload), {
                headers: { 'Content-Type': 'application/json' },
                tags: {answer:`post`}
            });
    ...
    let leaderboardResponse = http.get('http://localhost:5000/api/http/get?page=1&size=15&tid=01904abe-21fc-7e80-bf36-acfe5d96f27e', {tags: {leaderboard:`get`}});
